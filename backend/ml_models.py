...
# def train_and_save_model():
#     # Define paths relative to the location of this script (ml_models.py)
#     # Since ml_models.py is in 'backend', and 'models' and 'data' are inside 'backend'
...
#         print("Data loaded successfully.")
#     except FileNotFoundError as e:
#         print(f"Error: {e}. Please ensure both CSV files are in the {data_dir} directory.")
#         return
...
def train_and_evaluate_individual_models():
    # Define paths relative to the location of this script (ml_models.py)
    data_dir = 'data'
    models_dir = 'models'
    ...
    models = {
        ...
        "Stacking Classifier": StackingClassifier(
            ...
            cv=5,
            n_jobs=-1,
            verbose=0 # Set to 0 to suppress verbose output for cleaner results
        )
    }
    ...
    # Save the Stacking Classifier as the primary model for the API
    primary_model = models["Stacking Classifier"]
    model_path = os.path.join(models_dir, 'parkinsons_predictor.pkl')
    joblib.dump(primary_model, model_path)
    print(f"\nPrimary model (Stacking Classifier) saved to {model_path}")

    return results

if __name__ == "__main__":
    evaluation_results = train_and_evaluate_individual_models()
