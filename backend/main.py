...
print(f"Loaded HUGGINGFACE_API_KEY: {'*' * len(HUGGINGFACE_API_KEY) if HUGGINGFACE_API_KEY else 'NOT_LOADED'}\n")
...
origins = [
    "http://127.0.0.1:5501",
    # "http://127.0.0.1:5501/frontend/prediction.html",
    "http://localhost",
    "http://localhost:8000",
    "http://localhost:5500",
    "http://127.0.0.1",
...
#     # --- CHANGE 1: Switched to a more reliable free-tier model ---
#     # "mistralai/mistral-7b-instruct:free" can be overloaded.
#     # "openchat/openchat-7b:free" is also a high-performing free model 
#     # that is often more stable and better at following complex instructions.
#     OPENROUTER_MODEL = "mistralai/mistral-7b-instruct:free"
#     # You can also try "google/gemma-7b-it:free" if this one is slow

#     # --- CHANGE 2: Improved and clarified the system prompt ---
#     # This prompt is clearer, better structured, and explicitly
#     # tells the AI *not* to use the strikethrough formatting.
#     messages = [
#         {"role": "system", "content": """You are a helpful AI assistant specializing in Parkinson's Disease.

# **Your Role:**
# * Provide clear information about Parkinson's symptoms, facts, and common questions.
# * When a user provides prediction results (like probability or high 'MDVP:Jitter(%)'), **use that data** to make your summary more specific. For example: "You mentioned elevated jitter, which is a vocal characteristic that can be associated with Parkinson's."
# * Use Markdown (bullet points, bold text) for structured lists.

# **Strict Rules:**
# 1.  **NEVER Diagnose:** Do not, under any circumstances, diagnose the user or give medical advice.
# 2.  **Stay on Topic:** If asked about other medical conditions or unrelated topics, politely decline and state your expertise is limited to Parkinson's Disease.
# 3.  **No Strikethrough:** Do not use strikethrough text (~~like this~~).
# 4.  **Guide User:** If appropriate, gently guide the user to the "Get Prediction" section for a voice-based prediction.
# 5.  **Disclaimer:** ALWAYS end your response with this exact disclaimer: "Please remember, I am an AI assistant and not a medical professional. This information is for educational purposes only. You should consult a healthcare professional for any diagnosis or medical advice."
# """},
#     ]
...
#         "model": OPENROUTER_MODEL,
#         "messages": messages,
#         # --- CHANGE 3: Increased max_tokens ---
#         # 500 was too low, causing "incomplete output". 1500 is safer.
#         "max_tokens": 1500, 
#         "temperature": 0.7
#     }
...
#         }
#         try:
#             # --- CHANGE 4: Increased timeout ---
#             # 30s was too short for a free model, causing "blank space" errors.
#             response = await client.post("/chat/completions", json=payload, headers=headers, timeout=60.0) 
#             response.raise_for_status()
#             llm_response = response.json()
...
async def chat_with_llm(chat_message: ChatMessage):
    ...
    if not HUGGINGFACE_API_KEY:
        raise HTTPException(status_code=500, detail="HUGGINGFACE_API_KEY is not set in .env file.")

    # --- THE FINAL FIX ---
    # The correct base URL for Hugging Face's new universal router
    HUGGINGFACE_API_BASE = "https://router.huggingface.co/v1"
    
    # We will use Qwen 2.5 7B Instruct - it is highly reliable on the new router
...
1.  **NEVER Diagnose:** Do not, under any circumstances, diagnose the user or give medical advice.
2.  **Stay on Topic:** If asked about other medical conditions or unrelated topics, politely decline and state your expertise is limited to Parkinson's Disease.
3.  **CRITICAL RULE: DO NOT USE STRIKETHROUGH.** You must not, under any circumstances, use strikethrough text. Do not wrap your response in `~~`. This is a strict violation of your rules.
4.  **Guide User:** If appropriate, gently guide the user to the "Get Prediction" section for a voice-based prediction.
5.  **Disclaimer:** ALWAYS end your response with this exact disclaimer: "Please remember, I am an AI assistant and not a medical professional. This information is for educational purposes only. You should consult a healthcare professional for any diagnosis or medical advice."
"""},
    ]
    ...
    print(f"Sending request to Hugging Face with model: {HUGGINGFACE_MODEL}")
    
    # Adding a longer timeout in case the model has to "wake up"
    timeout_settings = httpx.Timeout(90.0, connect=60.0)
    
    async with httpx.AsyncClient(base_url=HUGGINGFACE_API_BASE, timeout=timeout_settings) as client:
        headers = {
            "Authorization": f"Bearer {HUGGINGFACE_API_KEY}",
            "Content-Type": "application/json"
        }
        try:
            # This will correctly POST to https://router.huggingface.co/v1/chat/completions
            response = await client.post("/chat/completions", json=payload, headers=headers)
            
            # Catch 503 if the model is waking up from sleep
            if response.status_code == 503:
                raise HTTPException(status_code=503, detail="The AI model is currently waking up on the server. Please try again in 20 seconds.")
            ...
            # Manually remove any strikethrough characters
            cleaned_response = chat_response_content.replace('~~', '')

            print(f"Received LLM response (cleaned): {cleaned_response[:100]}...")
            
            return {"response": cleaned_response} 
            
        except httpx.RequestError as e:
            print(f"HTTPX Request Error: {e}")
            raise HTTPException(status_code=500, detail=f"Could not connect to Hugging Face API: {e}")
        except httpx.HTTPStatusError as e:
            print(f"HTTP Status Error: {e.response.status_code} - {e.response.text}")
            raise HTTPException(status_code=e.response.status_code, detail=f"Hugging Face API error: {e.response.text}")
        except KeyError as e:
            print(f"KeyError in LLM response parsing: {e} - Response: {llm_response}")
            raise HTTPException(status_code=500, detail=f"Unexpected response format from Hugging Face API: {e}")
        except Exception as e:
            print(f"An unexpected error occurred during LLM interaction: {e}")
            raise HTTPException(status_code=500, detail=f"An unexpected error occurred: {e}")