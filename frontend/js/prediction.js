document.addEventListener("DOMContentLoaded", () => {
  ...
  function initUIEventListeners() {
    ...
    if (nav && mainContent) {
      mainContent.addEventListener('scroll', () => nav.classList.toggle('scrolled', mainContent.scrollTop > 10));
    }
  ...
  function initAccessibleTabs() {
    ...
    let initiallyActiveTab = tabList.querySelector('[role="tab"][aria-selected="true"]');
  ...
  async function handleStartRecording() {
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      if (errorMessageDiv) showMessage(errorMessageDiv, "Microphone access is not supported by your browser.", true);
      if (recordingStatus) recordingStatus.textContent = "Error: Feature not supported.";
    ...
    try {
      ...
      let userMessage = "Error: Could not access microphone. Please ensure it's connected and permissions are granted.";
      ...
      if (recordingStatus) recordingStatus.textContent = "Error: Mic access issue.";
  ...
  async function handleFormSubmit(event) {
    ...
    if (!isValid && firstInvalidField) {
        ...
        if (fieldTabPane && fieldTabPane.hidden) {
            const tabButton = document.getElementById(fieldTabPane.getAttribute('aria-labelledby'));
            if(tabButton) activateTab(tabButton, false);
        }
        showMessage(errorMessageDiv, `Please enter a valid number for <strong>${firstInvalidField.labels[0]?.textContent || firstInvalidField.name}</strong>.`, true);
    ...
    try {
      ...
      localStorage.setItem("parkinsonsPrediction", JSON.stringify({
      ...
      showMessage(errorMessageDiv, `Prediction failed: ${error.message}. Please check your inputs and try again.`, true);
    } finally {
      if (submitButton) {
  ...
  function visualize() {
    ...
    const draw = () => {
      ...
      visualizerContext.strokeStyle = `rgba(${getComputedStyle(document.documentElement).getPropertyValue('--color-primary-rgb')}, 0.7)`;
  ...
  async function sendAudioToBackend(audioBlob) {
    ...
    try {
      const response = await fetch("http://localhost:8000/extract-features", { method: "POST", body: formData });
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
      ...
      if (data.features) {
        ...
        if (recordingStatus) recordingStatus.textContent = "Features extracted. Ready for prediction.";
        if (tabs.length > 0) activateTab(tabs[0], true);
      } else {
        throw new Error(data.message || "Backend did not return features.");
      }
    } catch (error) {
      showMessage(errorMessageDiv, `Audio processing failed: ${error.message}. Please try manual entry or record again.`, true);
      if (recordingStatus) recordingStatus.textContent = "Error processing audio.";
    } finally {
  ...
  function populateFormWithFeatures(features) {
    ...
    Object.keys(features).forEach(key => {
        ...
        if (inputElement) {
            ...
            inputElement.style.backgroundColor = `rgba(${getComputedStyle(document.documentElement).getPropertyValue('--color-primary-rgb')}, 0.08)`;
        }
    });
    if (tabs.length > 0) {
        const firstPopulatedInput = document.getElementById(Object.keys(features)[0]);
        const tabPane = firstPopulatedInput?.closest('.tab-pane');
        const tabButton = tabPane ? document.getElementById(tabPane.getAttribute('aria-labelledby')) : tabs[0];
